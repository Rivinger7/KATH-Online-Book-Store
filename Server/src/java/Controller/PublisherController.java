    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            try {
                ...
                int total = bookDAO.countTotalBookByPublisherID(Integer.parseInt(publisherID));
                int endPage = total / 9;
                if (total % 9 != 0) {
                    ++endPage;
                }
                List<BookDTO> bookList = bookDAO.pagingBookListByPublisherID(Integer.parseInt(page), Integer.parseInt(publisherID));
